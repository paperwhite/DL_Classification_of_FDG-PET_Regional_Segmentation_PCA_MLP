...
path='/home/shibanis/Downloads/ss/build_ad_em'
...
for x in range(0,79*95*79):
    #t[x,0], p[x,0]=ttest_ind(emci_voxels[x],ad_voxels[x], equal_var=False)
    t[x], p[x]=ttest_ind(emci_voxels[x],ad_voxels[x], equal_var=False)
    if(p[x]<=0.05):
        p[x]=1
    else:
        p[x]=0
#t, p = ttest_ind(mu_AD, mu_EMCI, equal_var=False)
print p
...
new_image_t.to_filename('t_value_ad_em.nii.gz')
new_image_p = nb.Nifti1Image(np.reshape(p,(79,95,79)), affine=img.affine)
new_image_p.to_filename('thresholded_p_value_ad_em.nii.gz')
